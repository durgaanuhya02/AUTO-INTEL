...
class AnalystAgent(BaseAgent):
    ...
    async def process(self):
        ...
        try:
            ...
            anomalies, self.pending_anomalies = self.pending_anomalies, []
            for anomaly in anomalies:
                try:
                    analysis = await self._perform_root_cause_analysis(anomaly)
                except (KeyError, TypeError) as e:
                    self.logger.error(f"Skipping malformed anomaly: {e!r}")
                    continue
                if analysis:
    ...
    async def _perform_root_cause_analysis(self, anomaly: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            if self.openai_client:
                ...
                response = self._rule_based_analysis(anomaly, correlations)
            analysis_result = {
                "anomaly": anomaly,
                "root_cause_analysis": response,
                "correlations": correlations,
                "confidence_score": await self._calculate_analysis_confidence(anomaly, correlations),
    ...
    def _rule_based_analysis(self, anomaly: Dict[str, Any], correlations: Dict[str, float]) -> Dict[str, Any]:
        """Deterministic root-cause analysis used when no LLM is configured."""
        metric = anomaly["metric_type"]
        current = float(anomaly["current_value"])
        expected = float(anomaly.get("expected_value", current))
        direction = "below" if current < expected else "above"
        deviation = abs(current - expected) / abs(expected) if expected else 0.0

        related = [(m, c) for m, c in correlations.items() if abs(c) >= 0.4][:3]
        causes = [
            f"{m} shift ({'positively' if c > 0 else 'negatively'} correlated with {metric}, r={c:+.2f})"
            for m, c in related
        ] or [f"Unexplained {direction}-baseline deviation in {metric}"]

        return {
            "likely_causes": causes,
            "contributing_factors": [f"{metric} is {deviation:.0%} {direction} its expected value"],
            "business_impact": "high" if deviation > 0.3 else "moderate",
            "recommendations": [f"Investigate {m} alongside {metric}" for m, _ in related]
            or [f"Review recent changes affecting {metric}"],
            "source": "rule_based",
        }
